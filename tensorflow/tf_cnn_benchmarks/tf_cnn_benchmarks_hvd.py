...
tf.flags.DEFINE_string('model', 'googlenet', 'name of the model to run')
...
tf.flags.DEFINE_boolean('print_training_accuracy', True, """whether to
...
tf.flags.DEFINE_string(
    ...
    ('The method for managing variables: '
     'parameter_server, replicated, distributed_replicated, independent, horovod'))
...
class BenchmarkCNN(object):
  ...
  def __init__(self):
    ...
    if FLAGS.variable_update == 'horovod' and FLAGS.num_gpus > 1:
      raise ValueError('Horovod benchmarks require num_gpus=1 on each worker')

    if FLAGS.variable_update == 'horovod' and FLAGS.job_name:
  ...
  def print_info(self):
    ...
    if FLAGS.variable_update == 'horovod' and FLAGS.horovod_device:
      log_fn('Horovod on:  %s' % FLAGS.horovod_device)
    log_fn('==========')
  ...
  def add_forward_pass_and_gradients(
    ...
    with tf.device(self.devices[device_num]):
      ...
      if FLAGS.variable_update == 'horovod':
        import horovod.tensorflow as hvd
        if FLAGS.horovod_device:
          horovod_device = '/%s:0' % FLAGS.horovod_device
        else:
          horovod_device = ''
